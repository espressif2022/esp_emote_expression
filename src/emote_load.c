...
static esp_err_t emote_find_data_by_name(emote_handle_t handle, const char *name,
...
static esp_err_t emote_find_data_by_key(emote_handle_t handle, assets_hash_table_t *ht, const char *key, void **result)
{
    ...
    *result = emote_assets_table_get(ht, key);
    ESP_GOTO_ON_FALSE(*result, ESP_ERR_NOT_FOUND, error, TAG, "table:%s not found: %s", ht->name ? ht->name : "unknown", key);
...
static esp_err_t emote_load_assets_handle(emote_handle_t handle, const emote_data_t *data)
{
    esp_err_t ret = ESP_OK;
    mmap_assets_config_t asset_config;
    mmap_assets_handle_t *target_handle = NULL;
    int num = 0;
    ...
    target_handle = &handle->assets_handle;
...
static esp_err_t emote_load_emojis(emote_handle_t handle, cJSON *root)
{
    ...
    for (int i = 0; i < emojiCount; i++) {
        ...
        size_t emojiSize = 0;
        ret = emote_find_data_by_name(handle, file->valuestring, &emojiData, &emojiSize);
        if (ret != ESP_OK) {
...
static esp_err_t emote_load_icons(emote_handle_t handle, cJSON *root)
{
    ...
    for (int i = 0; i < iconCount; i++) {
        ...
        size_t iconSize = 0;
        ret = emote_find_data_by_name(handle, file->valuestring, &iconData, &iconSize);
        if (ret != ESP_OK) {
...
static esp_err_t emote_load_fonts(emote_handle_t handle, cJSON *root)
{
    ...
    ret = emote_find_data_by_name(handle, fontsTextFile, &fontData, &fontSize);
...
static esp_err_t emote_load_assets_data(emote_handle_t handle)
{
    esp_err_t ret = ESP_OK;
    ...
    ret = emote_find_data_by_name(handle, EMOTE_INDEX_JSON_FILENAME, &asset_data, &asset_size);
...
esp_err_t emote_load_assets_from_source(emote_handle_t handle, const emote_data_t *data)
{
    ...
    ret = emote_load_assets_handle(handle, data);
    ESP_GOTO_ON_FALSE(ret == ESP_OK, ret, error, TAG, "Failed to load assets handle");

    ret = emote_load_assets_data(handle);
